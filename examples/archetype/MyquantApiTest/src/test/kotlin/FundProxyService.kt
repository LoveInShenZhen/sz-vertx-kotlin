...
//
class FundProxyService : DsProxyEnvBase() {

    @Test
    @DisplayName("查询股票分红送配")
    fun GetDividend_stk() {
        val req = FundStkServiceProto.GetDividendReq.newBuilder().apply {
            symbol = "SHSE.601318"
    ...
    fun GetDividend_fund_dvd() {
        val req = FundFndServiceProto.FndGetDividendReq.newBuilder().apply {
            fund = "SZSE.161911"
            startDate = "1980-01-01"
            endDate = "2023-12-31"
        }.build()
    ...
    fun GetSplit_fund() {
        val req = FundFndServiceProto.GetSplitReq.newBuilder().apply {
            fund = "SHSE.516160"
