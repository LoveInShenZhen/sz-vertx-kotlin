...
class InstrumentService : ProdEnvBase() {
    ...
    @Test
    fun TestGetSymbols_CZCE() {
        val req = InstrumentServiceProto.GetSymbolsReq.newBuilder().apply {
            secType1 = 1040
            tradeDate = "2014-05-22"
        }.build()

        val rsp = instrument_api.getSymbols(req)
        rsp.symbolsList.filter { it.info.symbol == "CZCE.CF411" }.forEach {
            logger.info("${it.info.symbol}, ${it.info.secName}, ${it.info.secAbbr}, ${it.info.listedDate.toLocalDate()} ~ ${it.info.delistedDate.toLocalDate()}")
        }
    }
    ...
    fun TestGetSymbols_by_symbol() {
        MeasureTime {
            val req = InstrumentServiceProto.GetSymbolsReq.newBuilder().apply {
                secType1 = 1030
                addSymbols("SHSE.RR888880")
            }.build()

            val rsp = instrument_api.getSymbols(req)

            logger.info(rsp.pbToJsonStr())
            logger.info(rsp.symbolsList.first().info.secName)
        }
    }
...
//    }
    ...
    fun TestGetTradingDatesByYear_1980() {
        MeasureTime {
            val req = InstrumentServiceProto.GetTradingDatesByYearReq.newBuilder().apply {
                startYear = 1980
                endYear = 1980
            }.build()
    ...
    fun TestGetTradingDatesPrevN_1() {
        MeasureTime {
            ...
            val req = InstrumentServiceProto.GetTradingDatesPrevNReq.newBuilder().apply {
                date = "2023-09-08"
                n = 1
            }.build()
    ...
    fun TestGetTradingDatesPrevN_outof_range() {
        val req1 = InstrumentServiceProto.GetTradingDatesPrevNReq.newBuilder().apply {
            date = "2029-01-01"
            n = 10
        }.build()
        ...
        val req2 = InstrumentServiceProto.GetTradingDatesPrevNReq.newBuilder().apply {
            date = "1980-01-01"
            n = 10
        }.build()
    ...
    fun TestGetSymbolInfos_by_sec_type1() {
        MeasureTime {
            val req = InstrumentServiceProto.GetSymbolInfosReq.newBuilder().apply {
                secType1 = 1040
            }.build()
    ...
    fun Debut_test() {
        ...
        val rsp = instrument_api.getHistorySymbol(req)
        rsp.symbolsList.forEach {
            logger.info("------ ${it.info.symbol} ${it.tradeDate.toLocalDate()} ------")
            logger.info(it.pbToJsonStr())
            logger.info("是否ST: ${it.isSt}")
        }
    }
}